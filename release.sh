...
if ! curl --fail --location --silent --show-error --header "${auth}" --header "${accept}" https://api.github.com/repos/phronmophobic/skia-build/releases/tags/${release} > release.json ; then
  echo "> Creating release ${release}"
  curl --fail --location --silent --show-error --header "${auth}" --header "${accept}" --request POST \
    --data "{\"tag_name\":\"${release}\",\"name\":\"${release}\"}" \
    https://api.github.com/repos/phronmophobic/skia-build/releases > release.json
else
